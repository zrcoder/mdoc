...
func New(cfg *model.Config) (*Store, error) {
...
func (s *Store) getDocs() map[string]*Doc {
	return s.docs.Load().(map[string]*Doc)
}

func (s *Store) setDocs(docs map[string]*Doc) {
	s.docs.Store(docs)
}
func (s *Store) setDocsMemo(memo map[string]map[string]*Doc) {
	s.docsMemo.Store(memo)
}
func (s *Store) getDocsMemo() map[string]map[string]*Doc {
...
func (s *Store) Doc(language string) *Doc {
	docs := s.getDocs()
...
// node exists or the node content is empty, it fallbacks to use the node with
// samepath in default language.
func (s *Store) Match(language, path string) (d *Doc, fallback bool, err error) {
